...
export const InvoiceDocument = forwardRef<HTMLDivElement, InvoiceDocumentProps>(
  function InvoiceDocument({ invoice, customer, settings }, ref) {
    ...
    return (
      <div
        ref={ref}
        className="invoice-print-root bg-white text-black p-8 md:p-10 mx-auto w-full max-w-[210mm] text-[13px] leading-snug shadow-sm border border-neutral-200 print:shadow-none print:border-0 print:max-w-none"
      >
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6 border-b-2 border-black pb-6">
          ...
          <div className="text-left sm:text-right">
            ...
            <p className="mt-1">Date: {invoice.date}</p>
            <p>Due: {invoice.dueDate}</p>
            {invoice.reference ? <p className="mt-1">Ref / PO: {invoice.reference}</p> : null}
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 py-6">
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-neutral-500">Bill to</p>
          ...
          <div className="sm:text-right text-left">
            <p className="text-xs font-bold uppercase tracking-wide text-neutral-500">Amount due</p>
            <p className="mt-1 text-2xl font-bold">{formatMoney(Number(invoice.dueAmount), currency)}</p>
            <p className="mt-1 text-neutral-600">Paid: {formatMoney(Number(invoice.paidAmount), currency)}</p>
        ...
        <div className="mt-6 flex justify-end">
          <div className="w-full max-w-xs space-y-1 text-sm">
            ...
            <div className="flex justify-between border-b border-neutral-200 py-1">
              <span>VAT</span>
              <span className="font-medium">{formatMoney(Number(invoice.totalVat), currency)}</span>
            </div>
            {Number(invoice.totalDiscount) > 0 ? (
              <div className="flex justify-between border-b border-neutral-200 py-1">
                <span>Discount</span>
                <span className="font-medium">−{formatMoney(Number(invoice.totalDiscount), currency)}</span>
              </div>
            ) : null}
            <div className="flex justify-between py-2 text-base font-bold border-t-2 border-black">
              <span>Grand total</span>
              <span>{formatMoney(Number(invoice.grandTotal), currency)}</span>
            </div>
          </div>
        </div>
        ...
        {invoice.notes ? (
          <div className="mt-8 pt-6 border-t border-neutral-200">
            ...
            <p className="mt-2 whitespace-pre-line text-neutral-800">{invoice.notes}</p>
          </div>
        ) : null}
      </div>
    );
